...
function multasSlidePlanoAcao(state, empId) {
  ...
  const motivosRows = motivos.length
    ? motivos.map(m => '<div class="slide-plano__table-row">'
        + '<div class="slide-plano__table-cell" style="flex:0 0 52px;text-align:center;font-weight:700;">' + _multasPadZero2(m.qtde) + '</div>'
        + '<div class="slide-plano__table-cell" style="flex:1;font-weight:600;">' + (m.descricao||'') + '</div>'
        + '<div class="slide-plano__table-cell" style="flex:0 0 110px;text-align:right;color:#c0392b;font-weight:700;border-right:none;">' + multasFmtMoeda(m.valor) + '</div>'
        + '</div>').join('')
    : '<div class="slide-plano__table-type" style="color:#aaa;font-style:italic;">Sem motivos registrados</div>';
  ...
  return '<div class="multas-slide" id="multas-slide-plano-' + empId + '">'
    ...
    +     '<div class="multas-slide-secao" style="margin-bottom:0;">Plano de Ação</div>'
    +     '<img src="' + logoUrl + '" style="height:28px;object-fit:contain;" alt="' + (MULTAS_EMP_LABEL[empId]||'') + '">'
    +   '</div>'
    +   '<div class="slide-plano__table" style="flex-shrink:0;">'
    +     '<div class="slide-plano__table-hd" style="background:' + corEmp + ';color:#fff;">MOTIVOS — ' + mesLabel.toUpperCase() + '</div>'
    +     '<div style="display:flex;background:#e8e8e8;padding:4px 10px;border-bottom:1px solid #ccc;">'
    +       '<span style="flex:0 0 52px;font-size:9px;font-weight:700;color:#555;text-align:center;">QTDE</span>'
    +       '<span style="flex:1;font-size:9px;font-weight:700;color:#555;">DESCRIÇÃO</span>'
    +       '<span style="flex:0 0 110px;font-size:9px;font-weight:700;color:#555;text-align:right;">VALOR</span>'
    +     '</div>'
    +     motivosRows
    +     '<div class="slide-plano__table-sub">Plano de Ação — ' + mesLabel + '</div>'
    +   '</div>'
    +   '<div class="slide-plano__actions" style="flex:1;overflow:hidden;margin-top:4px;">' + bullets + '</div>'
    + '</div>'
    + multasSlideFooter(state.dataAtualizacao, state.analista)
    + '</div>';
}
...
function multasRenderStep2(state, containerId) {
  ...
  // Tabs empresa
  var EMP_CORES = { grupo:'#1d3061', '1001':'#9e9e9e', '1001u':'#bdbdbd', cat:'#03a5a5', com:'#4a90d9' };
  var empTabs = ['grupo','1001','1001u','cat','com'].map(function(id) {
    var label = MULTAS_EMP_LABEL[id];
    var active = id === tabAtiva;
    var cor = EMP_CORES[id] || '#1d3061';
    var style = active
      ? 'background:' + cor + ';color:#fff;border-color:' + cor + ';font-weight:700;'
      : 'background:#fff;color:' + cor + ';border-color:' + cor + ';';
    return '<button class="sin-form-tab" style="' + style + '"'
...
function multasCopiarLinkGeral() {
  ...
  var url  = base + 'plano-de-acao-multas.html?mes=' + mes + '&ano=' + ano;
  navigator.clipboard.writeText(url).then(function() {
    alert('Link copiado!\n\n' + url + '\n\nEnvie este link para todas as empresas responderem.');
  }).catch(function() {
...
function multasBuildSlidesList() {
  ...
  ['1001','1001u','cat','com'].forEach(function(empId) {
    ...
    list.push({ type: 'op-emp',   empId: empId });
    list.push({ type: 'motivos',  empId: empId });
    list.push({ type: 'plano',    empId: empId });
  });
