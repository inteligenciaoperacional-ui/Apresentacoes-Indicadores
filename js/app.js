...
document.addEventListener("click", function(e) {
  // KM Fretamento — delegação por data-km-action
  var kmEl = e.target.closest("[data-km-action]");
  if (kmEl) {
    if (typeof kmHandleAction === "function" && typeof kmState !== "undefined" && kmState) {
      kmHandleAction(kmEl.dataset.kmAction, kmEl.dataset, kmEl);
    } else if (kmEl.dataset.kmAction === "go-home") {
      // fallback caso kmState ainda não exista
      state.screen = "home"; render();
    }
    return;
  }
  // Reclamações de Clientes — delegação por data-rc-action
  var rcEl = e.target.closest("[data-rc-action]");
  if (rcEl) {
    if (typeof rcHandleAction === "function" && typeof rcState !== "undefined" && rcState) {
      rcHandleAction(rcEl.dataset.rcAction, rcEl.dataset, rcEl);
    } else if (rcEl.dataset.rcAction === "go-home") {
      state.screen = "home"; render();
    }
    return;
  }
  // Multas de Trânsito — delegação por data-mt-action
  var mtEl = e.target.closest("[data-mt-action]");
  if (mtEl) {
    if (typeof mtHandleAction === "function" && typeof mtState !== "undefined" && mtState) {
      mtHandleAction(mtEl.dataset.mtAction, mtEl.dataset, mtEl);
    } else if (mtEl.dataset.mtAction === "go-home") {
      state.screen = "home"; render();
    }
    return;
  }
  // Eficiência Diesel — delegação por data-diesel-action
  var dieselEl = e.target.closest("[data-diesel-action]");
  if (dieselEl) {
    if (typeof dieselHandleAction === "function" && typeof dieselState !== "undefined" && dieselState) {
      dieselHandleAction(dieselEl.dataset.dieselAction, dieselEl.dataset, dieselEl);
    } else if (dieselEl.dataset.dieselAction === "go-home") {
      state.screen = "home"; render();
    }
    return;
  }
...
document.addEventListener("input", function(e) {
  ...
  // KM Fretamento
  if (el && (el.dataset.kmField || el.dataset.kmRoot || el.dataset.kmMes || el.dataset.kmContrato || el.dataset.kmEvol)) {
    if (typeof kmHandleInput === "function" && typeof kmState !== "undefined" && kmState) {
      kmHandleInput(el);
    }
    return;
  }
  ...
  // Multas de Trânsito
  if (el && (el.dataset.mtRoot || el.dataset.mtGrupo || el.dataset.mtCausaIdx ||
      el.dataset.mtDiaIdx || el.dataset.mtEndPrefix || el.dataset.mtVelF ||
      el.dataset.mtVelEmp || el.dataset.mtEmp || el.dataset.mtEmpF ||
      el.dataset.mtCustoF || el.dataset.mtMotivoIdx || el.dataset.mtLocalIdx)) {
    if (typeof mtHandleInput === "function" && typeof mtState !== "undefined" && mtState) {
      mtHandleInput(el);
    }
    return;
  }
  ...
  if (el && (el.dataset.rcRoot || el.dataset.rcEmp || el.dataset.rcGrupoF ||
      el.dataset.rcGrupo || el.dataset.rcMotacumIdx !== undefined ||
      el.dataset.rcMotmesIdx !== undefined || el.dataset.rcKpiEmp ||
      el.dataset.rcElogios || el.dataset.rcElogiosKpi ||
      el.dataset.rcElogiosMesemp !== undefined || el.dataset.rcPlanoEmp)) {
    if (typeof rcHandleInput === "function" && typeof rcState !== "undefined" && rcState) {
      rcHandleInput(el);
    }
    return;
  }
  ...
  // Eficiência Diesel
  if (el && (el.dataset.dieselRoot || el.dataset.dieselField)) {
    if (typeof dieselState !== "undefined" && dieselState) {
      if (el.dataset.dieselRoot) {
        var f = el.dataset.dieselRoot;
        if (f === "mes")      dieselState.mes      = parseInt(el.value);
        else if (f === "ano") dieselState.ano      = parseInt(el.value);
        else                  dieselState[f]       = el.value;
      } else {
        if (typeof dieselHandleInput === "function") dieselHandleInput(el);
      }
    }
    return;
  }
  // Contratos diesel
  if (el && el.dataset.dieselContratoIdx !== undefined && typeof dieselState !== "undefined" && dieselState) {
    if (typeof dieselHandleContratoInput === "function") dieselHandleContratoInput(el);
    return;
  }
  ...
  // Inputs do módulo Multas (Step 1)
  if (el && el.dataset.multasRoot) {
    if (multasState) {
      var field = el.dataset.multasRoot;
      if (field === "mes") multasState.mes = parseInt(el.value);
      else if (field === "ano") multasState.ano = parseInt(el.value);
      else multasState[field] = el.value;
    }
    return;
  }
...
function _handleAction(action, dataset, el) {
  switch (action) {
    ...
    case "start-km-fretamento":
      startKmFretamento(); break;

    case "start-multas-transito":
      startMultasTransito(); break;

    case "start-reclamacoes-cliente":
      startReclamacoesCliente(); break;

    case "start-eficiencia-diesel":
      startEficienciaDiesel(); break;

    case "multas-go-step":
      multasRenderStep(Number(dataset.step)); break;

    case "multas-go-slide":
      if (multasState) {
        multasState.slideIdx = Number(dataset.idx);
        multasRenderCurrentSlide();
        multasUpdateSlideCounter();
        multasUpdateActiveThumbs();
      }
      break;

    case "multas-prev-slide":
      if (multasState && multasState.slideIdx > 0) {
        multasState.slideIdx--;
        multasRenderCurrentSlide();
        multasUpdateSlideCounter();
        multasUpdateActiveThumbs();
      }
    ...
    case "multas-next-slide":
      if (multasState && multasState.slides && multasState.slideIdx < multasState.slides.length - 1) {
        multasState.slideIdx++;
        multasRenderCurrentSlide();
        multasUpdateSlideCounter();
        multasUpdateActiveThumbs();
    ...
    case "multas-set-tab":
      if (multasState) {
        multasState._tabAtiva = dataset.multasTab;
        multasRenderStep2(multasState, "multas-step2-content");
      }
      break;

    case "multas-salvar-firebase":
      _multasSalvarManual(); break;

    case "multas-importar-planos":
      _multasImportarPlanos(); break;

    case "multas-gerar-pdf":
      multasGerarPdfCompleto(multasState); break;
