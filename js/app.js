...
function render() {
  ...
  // Módulo Pontualidade tem prioridade quando ativo
  if (typeof pontState !== "undefined" && pontState) {
    app.innerHTML = pontRender();
    if (pontState.screen === "step3") {
      setTimeout(function() { _pontRenderSlide(); }, 50);
    }
    return;
  }

  if (state.screen === "home") {
    app.innerHTML = homeHTML(); return;
  }
...
document.addEventListener("click", function(e) {
  // Pontualidade
  var elP = e.target.closest("[data-pont-action]");
  if (elP) { if (typeof pontHandleAction === "function" && typeof pontState !== "undefined" && pontState) { pontHandleAction(elP.dataset.pontAction, elP.dataset, elP); } else if (elP.dataset.pontAction === "go-home") { state.screen = "home"; render(); } return; }

  // Diesel
  var elD = e.target.closest("[data-diesel-action]");
  if (elD) { if (typeof dieselHandleAction === "function") dieselHandleAction(elD.dataset.dieselAction, elD.dataset, elD); return; }

  // KM Fretamento
  var elK = e.target.closest("[data-km-action]");
  if (elK) { if (typeof kmHandleAction === "function") kmHandleAction(elK.dataset.kmAction, elK.dataset, elK); return; }

  // Reclamações de Clientes
  var elR = e.target.closest("[data-rc-action]");
  if (elR) { if (typeof rcHandleAction === "function") rcHandleAction(elR.dataset.rcAction, elR.dataset, elR); return; }

  // Multas Trânsito
  var elMT = e.target.closest("[data-mt-action]");
  if (elMT) { if (typeof mtHandleAction === "function") mtHandleAction(elMT.dataset.mtAction, elMT.dataset, elMT); return; }
...
document.addEventListener("change", function(e) {
  ...
  // Selects do módulo pontualidade (seta regional)
  if (el.dataset.pontSlide && el.dataset.pontRegIdx !== undefined && el.dataset.pontRegField) {
    if (typeof pontHandleInput === "function" && pontState) { pontHandleInput(el); }
    return;
  }
});
...
document.addEventListener("input", function(e) {
  ...
  // Inputs do modulo Pontualidade
  if (el && (el.dataset.pontSlide || el.dataset.pontKpi || el.dataset.pontEmpIdx !== undefined ||
      el.dataset.pontRegIdx !== undefined || el.dataset.pontGrafIdx !== undefined || el.dataset.pontField)) {
    if (typeof pontHandleInput === "function" && pontState) {
      if (typeof _pontApplyFormat === "function") _pontApplyFormat(el);
      pontHandleInput(el);
    }
    return;
  }
...
function _handleAction(action, dataset, el) {
  switch (action) {
    case "home-cat":
      _homeCategoria = dataset.cat;
      render();
      break;

    case "start-pontualidade":
      startPontualidade(); break;

    case "go-home":
      state.screen = "home"; state.step = 1; pontState = null; render(); break;
    ...
    case "multas-importar-sheets":
      _multasImportarSheets(); break;
