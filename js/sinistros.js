        ...
        legend: {
          ...
          labels: {
            ...
            filter: function(item) {
              // Oculta "Sinistros Manutencao" quando não há dados de manutenção
              if (item.text === "Sinistros Manutencao" && !withManut) return false;
              return true;
            }
          }
        },
...
function _sinMakeDonutConfig(gravidade, animOn, corEmpresa) {
  ...
  // Garante arco mínimo de 8% para fatias com dados, preservando proporção visual
  // Os valores reais são exibidos nos labels — os dados do gráfico são só visuais
  var rawVals = [l, m, g, gg];
  var MIN_PCT = 8; // % mínimo de arco por fatia com valor > 0
  var activeCount = rawVals.filter(function(v){ return v > 0; }).length;
  var minVal = total * (MIN_PCT / 100);
  // Só aplica mínimo se houver mais de 1 fatia ativa
  var visualVals = activeCount > 1 ? rawVals.map(function(v) {
    return v > 0 ? Math.max(v, minVal) : 0;
  }) : rawVals;
    ...
    options: {
      ...
      plugins: {
        ...
        datalabels: {
          anchor: "center", align: "center",
          color: function(ctx) {
            // Grave (idx 2) e Gravissimo (idx 3) têm fundo escuro → texto branco
            // Leve (idx 0) e Medio (idx 1) têm fundo claro → texto escuro
            return ctx.dataIndex >= 2 ? "#ffffff" : "#333333";
          },
          font: function(ctx) {
            var v = ctx.dataset.data[ctx.dataIndex];
            var pct = v / total * 100;
            return { size: pct < 8 ? 8 : 11, weight: "bold" };
          },
...
function _sinMakeHBarConfig(items, labelField, valueField, corBarra, animOn, kmField) {
  ...
  var kmPlugin = {
    ...
    afterDraw: function(chart) {
      ...
      var maxVal = Math.max.apply(null, ds.data.filter(function(x){ return x > 0; })) || 1;

      // Labels dos valores das barras — sempre azul escuro na extremidade interna
      ds.data.forEach(function(v, i) {
        if (!v) return;
        var cy   = yAxis.getPixelForTick(i);
        var xBar = xAxis.getPixelForValue(v);
        var txt  = v.toLocaleString("pt-BR");
        ctx.save();
        ctx.font         = "bold 10px Arial,sans-serif";
        ctx.textBaseline = "middle";
        ctx.fillStyle    = "#1d3061";
        ctx.textAlign    = "right";
        ctx.fillText(txt, xBar - 6, cy);
        ctx.restore();
      });

      // Labels de km (abaixo do nome da barra)
